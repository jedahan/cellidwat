...
@updateCellId = ->
  navigator.geolocation.getCurrentPosition (position) ->
    ...
    $.getJSON url + "&latmin=#{latmin}&latmax=#{latmax}&lonmin=#{lonmin}&lonmax=#{lonmax}", (response) ->
...
$(document).ready ->
    ...
    unless localStorage.cellid? and JSON.parse(localStorage.cellid)?.timestamp < (new Date()).getTime() - 2*60*1000
      updateCellId()
